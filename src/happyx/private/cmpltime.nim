...
when defined(js):
  proc exportRouteArgs*(urlPath, routePath, body: NimNode): NimNode {.compileTime.} =
    ...
    var
      ...
      hasChildren = false
    # integer param
    routePathStr = $routePathStr.replace(newRegExp"\{[a-zA-Z][a-zA-Z0-9_]*:int\}", "(\\d+)")
    # float param
    routePathStr = $routePathStr.replace(newRegExp"\{[a-zA-Z][a-zA-Z0-9_]*:float\}", "(\\d+\\.\\d+|\\d+)")
    # word param
    routePathStr = $routePathStr.replace(newRegExp"\{[a-zA-Z][a-zA-Z0-9_]*:word\}", "(\\w+?)")
    # string param
    routePathStr = $routePathStr.replace(newRegExp"\{[a-zA-Z][a-zA-Z0-9_]*:string\}", "([^/]+?)")
    # path param
    routePathStr = $routePathStr.replace(newRegExp"\{[a-zA-Z][a-zA-Z0-9_]*:path\}", "([\\S]+)")
    # path param
    routePathStr = $routePathStr.replace(newRegExp"\{[a-zA-Z][a-zA-Z0-9_]*:/([\s\S]+?)/\}", "($1)")

    let
      regExp = newCall("newRegExp", newStrLitNode("^" & routePathStr & "$"))
      found = path.match(newRegExp"\{([a-zA-Z][a-zA-Z0-9_]*):(int|float|string|path|word|/[\s\S]+?/)\}")
      foundLen = found.len
    ...
    for i in found:
      let
        name = ident(($i)[1..^1].split(":")[0])
        argTypeStr = ($i)[1..^1].split(":")[1]
