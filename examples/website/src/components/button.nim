...
component Button:
  ...
  `template`:
    ...
    tDiv(class = "flex justify-center items-center lowercase font-bold text-lg cursor-pointer select-none"):
      if self.flat:
        tDiv(class = "px-8 text-[{Foreground}] hover:opacity-80 active:opacity-60 transition-all duration-300"):
          slot
      else:
        tDiv(class = "px-8 text-[{Background}] rounded-full bg-[{Foreground}] hover:opacity-90 active:opacity-75 transition-all duration-300"):
          slot
      @click:
